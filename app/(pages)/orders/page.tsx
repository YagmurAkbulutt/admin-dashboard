    ...
    <tbody>
      {data.map((order,key) => (
        <tr key={key}>
          ...
          <td className="whitespace-nowrap">{new Date(order.order_date).toLocaleString("tr",{
            day:"2-digit", month:"long"
          })}</td>
          <td>{order.shipping_address.city}</td>
          <td >
            <span className={`${colors[order.status]} py-1 px-2  text-white rounded-lg shadow`}>{order.status}
            </span></td>
        </tr>
      ))}
    </tbody>
